...
/*
 * ***************************************************************************
 * File Description  : 天津生态城网站-服务公告 Controller
 * File Version      : 1.0
 ...
 * Original Filename : NoticeAction.class.php
 * Author            : lzhang
 ...
 * @version     1.0
 */
class NoticeAction extends HomeAction {
    /**
     * 共通设置顶部导航项目高亮显示
     */
    ...
    public function index() {
        ...
        $catIdArr = array('1' => 2, '2' => 211, '3' => 3, '4' => 161);
    ...
    public function view() {
        ...
        $notice = D('Article')->where($where)->find();
        if (empty($notice)) {
            $this->redirect("Public/404");
        }
        $notice['tags'] = explode(',', $notice['tag']);

        $this->assign('notice', $notice);

        $param['cat_id'] = $notice['cat_id'];
        $param['status'] = 1;
        $param['isre'] = 1;

        $param['start_time'] = array('gt', $notice['start_time']);
        $prevview = D('Article')->where($param)->limit(1)->order('start_time asc')->find();
        $this->assign('prevview', $prevview);

        $param['start_time'] = array('lt', $notice['start_time']);
        $nextview = D('Article')->where("status=1 and cat_id = " . $notice['cat_id'] . " and id < " . $id)->limit(1)->order('start_time DESC')->find();
        $this->assign('nextview', $nextview);

        // 导航
        $this->id = $id;
        $this->nav = $this->nav($this->cate, $notice['cat_id'], '');
