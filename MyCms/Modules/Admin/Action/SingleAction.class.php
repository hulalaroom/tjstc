...
class SingleAction extends AdminAction {
    ...
    public function index() {
		...
		$singlelist = $m_guest->limit($page->firstRow . ',' . $page->listRows)->relation(true)->order('id DESC')->select();
    ...
    public function update() {
      ...
      $r = D('Recoverysingle')->where("ID = ".$id)->setField($data);
    ...
    public function bat() {
        ...
        switch ($command) {
            case 'delete':
                $re = D('Recoverysingle')->where($map)->delete();
                $type = "删除";
                break;
            case 'status':
                $re = D('Recoverysingle')->where($map)->setField('status', '0');
                $type = "未受理";
                break;
            case 'alstatus':
                $re = D('Recoverysingle')->where($map)->setField('status', '1');
                $type = "已受理";
                break;
			case 'dlstatus':
                $re = D('Recoverysingle')->where($map)->setField('status', '2');
                $type = "已办结";
                break;
        }
        if ($re) {
